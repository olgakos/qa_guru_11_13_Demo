...
Шаги:
1. Открыть терминал
2. Запустить на выполение команду:
###### Локальный запуск:
```
gradle clean test
```
...
Шаги:
1. Загеристированным пользователем перейти на страницу сборки проекта по ссылке: <a target="_blank" href="https://jenkins.autotests.cloud/job/qa_guru_11_13_Demo/">Jenkins Itigris</a>
2. Перечисленные ниже параметры можно менять в графическом интерфейсе.
3. Запустить выполение тестов кнопкой "Собрать" (внизу страницы) или "Собрать с параметрами" (в меню слева)
...
Ожидаемый результат: Откроется страница с готовым Allure Report
